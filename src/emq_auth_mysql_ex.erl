...
-import(emq_auth_mysql_ex_cli, [is_superuser/2, query/3]).
...
check(Client=#mqtt_client{username = Username}, Password, #state{auth_query  = {AuthSql, AuthParams},
                               super_query = SuperQuery,
                               hash_type   = HashType}) ->
    Result = case query(AuthSql, AuthParams, Client) of
                 {ok, [<<"password">>, <<"max_online">>], [[PassHash, MaxOnline]]} ->
                     case check_pass(PassHash, Password, HashType) of
                        ok ->
                            emq_login_limit:check_limit(Username, MaxOnline);
                        {error, R1} ->
                            {error, R1}
                     end;
                 {ok, [<<"password">>, <<"salt">>, <<"max_online">>], [[PassHash, Salt, MaxOnline]]} ->
                     case check_pass(PassHash, Salt, Password, HashType) of
                        ok ->
                            emq_login_limit:check_limit(Username, MaxOnline);
                        {error, R2} ->
                            {error, R2}
